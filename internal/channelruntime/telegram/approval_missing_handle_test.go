...
func TestMarkTelegramMissingApprovalHandleDenyCancelsPendingTask(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("markTelegramMissingApprovalHandle() error = %v", err)
	}
	if taskID != "task_1" || resumed {
		t.Fatalf("result = taskID %q resumed %v, want task_1 false", taskID, resumed)
	}
	...
	if task.Error != telegramApprovalResultText(false) {
		t.Fatalf("task error = %q, want %q", task.Error, telegramApprovalResultText(false))
	}
