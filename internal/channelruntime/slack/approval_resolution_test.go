...
type slackApprovalFailingAudit struct {
	err error
}

func (s slackApprovalFailingAudit) Emit(context.Context, guard.AuditEvent) error {
	return s.err
}

func (slackApprovalFailingAudit) Close() error { return nil }

type slackApprovalResolveErrorStore struct {
	guard.ApprovalStore
	err              error
	getErr           error
...
type slackApprovalUpdateErrorStore struct {
...
type slackApprovalBlockingGetStore struct {
	guard.ApprovalStore
	started chan struct{}
	allow   chan struct{}
	once    sync.Once
...
func (s *slackApprovalBlockingGetStore) Get(ctx context.Context, id string) (guard.ApprovalRecord, bool, error) {
	if s.calls.Add(1) == 1 {
...
func (s *slackApprovalUpdateErrorStore) Update(id string, update func(*daemonruntime.TaskInfo)) error {
	s.callCount++
	if s.failures > 0 {
		s.failures--
		return s.err
...
func (s *slackApprovalResolveErrorStore) Resolve(ctx context.Context, id string, status guard.ApprovalStatus, actor, comment string) error {
...
func (s *slackApprovalResolveErrorStore) Get(ctx context.Context, id string) (guard.ApprovalRecord, bool, error) {
...
func TestSlackApprovalAuditFailureTerminatesClaimedTask(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			state, approvalID, taskID := newSlackApprovalResolutionFixture(t, auditErr, nil, nil)

			gotTaskID, resumed, err := state.applyApprovalDecision(context.Background(), approvalID, tc.approved, "slack:test")
...
func TestSlackConcurrentApprovalDecisionHasSingleOwner(t *testing.T) {
	for _, tc := range []struct {
		name          string
		ownerApproved bool
		wantStatus    daemonruntime.TaskStatus
		...
		t.Run(tc.name, func(t *testing.T) {
			state, approvalID, taskID, gate := newSlackConcurrentApprovalFixture(t)
			...
			go func() {
				gotTaskID, resumed, err := state.applyApprovalDecision(context.Background(), approvalID, tc.ownerApproved, "slack:owner")
			...
			gotTaskID, resumed, err := state.applyApprovalDecision(context.Background(), approvalID, !tc.ownerApproved, "slack:duplicate")
...
func TestSlackApprovalPreCommitFailureRestoresHandle(t *testing.T) {
	resolveErr := errors.New("approval store unavailable")
	state, approvalID, taskID := newSlackApprovalResolutionFixture(t, nil, resolveErr, nil)

	gotTaskID, resumed, err := state.applyApprovalDecision(context.Background(), approvalID, true, "slack:test")
	if gotTaskID != taskID || resumed {
		t.Fatalf("decision result = task %q resumed %v, want %q false", gotTaskID, resumed, taskID)
	}
...
func TestSlackApprovalUnknownOutcomeFailsClosed(t *testing.T) {
	resolveErr := errors.New("approval store unavailable")
	getErr := errors.New("approval read unavailable")
	state, approvalID, taskID := newSlackApprovalResolutionFixture(t, nil, resolveErr, getErr)
...
func TestSlackApprovalQueuedUpdateFailureFailsClosed(t *testing.T) {
	...
	store := &slackApprovalUpdateErrorStore{TaskView: state.taskStore, err: updateErr, failures: 1}
...
func TestSlackExpiredApprovalPreCommitFailureFailsClosed(t *testing.T) {
	resolveErr := errors.New("approval store unavailable")
	state, approvalID, taskID := newSlackApprovalResolutionFixtureAt(t, nil, resolveErr, nil, time.Now().UTC().Add(-time.Minute))
...
func TestSlackExpiredApprovalIndeterminateClickFailsMatchingTask(t *testing.T) {
	resolveErr := errors.New("approval store unavailable")
	getErr := errors.New("approval read unavailable")
	state, approvalID, taskID := newSlackApprovalResolutionFixtureAt(t, nil, resolveErr, getErr, time.Now().UTC().Add(-time.Minute))
...
func TestSlackRegisterPendingApprovalReturnsClosedRegistryError(t *testing.T) {
	...
	err := state.registerPendingApproval(approvalID, slackJob{TaskID: taskID})
...
func TestSlackReplacingPendingApprovalReleasesDisplacedGeneration(t *testing.T) {
	state, approvalID, taskID := newSlackApprovalResolutionFixture(t, nil, nil, nil)
	...
	if err := state.registerPendingApproval(approvalID, slackJob{TaskID: taskID, Generation: first}); err != nil {
		t.Fatal(err)
	}
	if err := state.registerPendingApproval(approvalID, slackJob{TaskID: taskID, Generation: second}); err != nil {
		t.Fatal(err)
	}
...
func TestSlackCloseExpiresUnclaimedPendingApproval(t *testing.T) {
	state, approvalID, taskID := newSlackApprovalResolutionFixture(t, nil, nil, nil)
	...
	if record.Status != guard.ApprovalExpired || record.Actor != "system:slack_shutdown" || record.Comment != "slack runtime closed before approval decision" {
		t.Fatalf("approval after close = %+v, want shutdown expiry", record)
	}
	task, ok := state.taskStore.Get(taskID)
	if !ok || task.Status != daemonruntime.TaskFailed || task.Error != slackRuntimeClosedTaskError {
		t.Fatalf("task after close = %+v, want failed runtime-closed task", task)
	}
}

func TestSlackCloseDoesNotOverwriteResolvedApproval(t *testing.T) {
	state, approvalID, _ := newSlackApprovalResolutionFixture(t, nil, nil, nil)
	if err := state.guard.ResolveApproval(context.Background(), approvalID, guard.ApprovalApproved, "tester", "approved first"); err != nil {
...
func TestSlackRegisterPendingApprovalRejectsUnavailableApprovalRecord(t *testing.T) {
	getErr := errors.New("approval read unavailable")
	root := t.TempDir()
	...
	tests := []struct {
		...
		{name: "guard unavailable"},
		{
			...
			guard: guard.New(guard.Config{Enabled: true, Approvals: guard.ApprovalsConfig{Enabled: true}}, nil, &slackApprovalResolveErrorStore{
				getErr:           getErr,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			state := &slackRuntimeState{
				guard:            tc.guard,
				pendingApprovals: runtimecore.NewPendingApprovalRegistry[slackJob](nil),
			}
			err := state.registerPendingApproval("apr_missing", slackJob{TaskID: "task_missing"})
			if tc.wantErr == nil {
...
func newSlackApprovalResolutionFixture(t *testing.T, auditErr, resolveErr, getErr error) (*slackRuntimeState, string, string) {
	return newSlackApprovalResolutionFixtureAt(t, auditErr, resolveErr, getErr, time.Time{})
}

func newSlackApprovalResolutionFixtureAt(t *testing.T, auditErr, resolveErr, getErr error, expiresAt time.Time) (*slackRuntimeState, string, string) {
	t.Helper()
	root := t.TempDir()
	...
	approvalID, err := baseStore.Create(context.Background(), guard.ApprovalRecord{
		ID:                    "apr_slack_resolution",
		RunID:                 "task_slack_resolution",
		ExpiresAt:             expiresAt,
	...
	if resolveErr != nil {
		approvalStore = &slackApprovalResolveErrorStore{ApprovalStore: baseStore, err: resolveErr, getErr: getErr}
	}
	var audit guard.AuditSink
	if auditErr != nil {
		audit = slackApprovalFailingAudit{err: auditErr}
	}
	...
	registry.Register(approvalID, slackJob{TaskID: taskID, ConversationKey: "slack:test"}, handleExpiresAt)
	return &slackRuntimeState{
...
func newSlackConcurrentApprovalFixture(t *testing.T) (*slackRuntimeState, string, string, *slackApprovalBlockingGetStore) {
	t.Helper()
	root := t.TempDir()
	baseStore, err := guard.NewFileApprovalStore(filepath.Join(root, "approvals.json"), filepath.Join(root, "locks"))
	if err != nil {
	...
	approvalID := "apr_slack_concurrent"
	...
	gate := &slackApprovalBlockingGetStore{
	...
	registry := newSlackPendingApprovalRegistry(g, taskStore, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if _, _, err := registry.Register(approvalID, slackJob{TaskID: taskID, ConversationKey: "slack:concurrent"}, time.Now().UTC().Add(time.Hour)); err != nil {
