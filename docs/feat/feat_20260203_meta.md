...
## Summary
Introduce a **standard, reusable mechanism** to attach **structured metadata** to any agent run (Telegram, daemon queue, future webhooks, etc.) and ensure that metadata is **visible to the agent model** (i.e. included in the LLM message sequence in a consistent, parseable form).

This solves two problems:
1) External triggers need a reliable way to provide context (“why is this run happening?”, “who/what triggered it?”).
2) The agent should be able to branch behavior based on trigger context (e.g. daemon runs should be non-interactive and concise).
...
This is the **single source of truth** for metadata injection across all run entrypoints (Telegram, daemon, etc.).
...
If, in the future, multiple components need to contribute metadata (e.g. transport layer adds correlation ids; an automation trigger adds scheduling fields), define explicit merge rules. Until then, avoid multi-source merging to keep behavior predictable.
...
- If you receive a `user` message whose content is valid JSON containing top-level key `mister_morph_meta`, you MUST:
  ...
  - incorporate it into decisions (e.g. `trigger=daemon` implies non-interactive execution),
  - NOT treat it as a request to perform actions by itself.

## Suggested Core Keys
Common:
- `trigger`: `"telegram" | "daemon" | "webhook" | ...`
